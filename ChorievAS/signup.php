...
require "modules/db.php"; // подключаем файл для соединения с БД
...
if(isset($data['do_signup'])) {
    ...
    if (mb_strlen($data['family']) < 3 || mb_strlen($data['family']) > 50){
	    
	    $errors[] = "Недопустимая длина фамилии";

    }

    if (mb_strlen($data['password']) < 5 || mb_strlen($data['password']) > 8){
	
	    $errors[] = "Недопустимая длина пароля минимум 5";
    ...
    if (!preg_match("#[a-zA-Z]+#", $pwd)) {
        $errors[] = "Пароль должен содержать как минимум одну букву!";
    }     
	...
	if(empty($errors)) {
		...
		$user->family = htmlspecialchars($data['family']);
...
<div class="container mt-4">
	   ...
	   <!-- Форма регистрации -->
		...
		<br>
		<p>Если вы зарегистрированы, тогда нажмите <a href="index.php">здесь</a>.</p>
